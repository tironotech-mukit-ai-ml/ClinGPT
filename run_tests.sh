...
def _load_ner_model():
    """Try NER models in priority order; return (nlp, model_name) or (None, None)."""
    try:
        import spacy
    except ImportError:
    ...
    for name in _NER_MODEL_PRIORITY:
        try:
            nlp = spacy.load(name)
            logger.info("NER layer: loaded model '%s'", name)
            return nlp, name
        except OSError:
            logger.debug("NER layer: model '%s' not found, trying next", name)

    logger.warning(
        "NER layer: no spaCy model found. "
        "Install scispaCy: pip install scispacy && pip install "
...
def _run_ner(text: str, nlp) -> list[_NerSpan]:
    ...
    for ent in doc.ents:
        ...
        if canonical is None:
            continue
        spans.append(_NerSpan(start=ent.start_char, end=ent.end_char,
                               entity_type=canonical, text=ent.text))

    logger.debug("NER layer: %d spans detected", len(spans))
    return spans
    ...
    def __init__(self):
        ...
        if self.enabled:
            try:
                ...
                self._nlp, self._ner_model_name = _load_ner_model()
                logger.info(
